...
reclass <- matrix(c(0,0.9,0,0.9,1,1),ncol=3,byrow=TRUE)

multi.spp <- raster(paste('D:/Climate_Strongholds/analyses_misc/',folder,'/',folder,'_ensemble_in_',season,'.img',sep=''))
# plot(multi.spp)
m90 <- reclassify(multi.spp, rcl=reclass)
plot(m90)
writeRaster(m90,paste('D:/Climate_Strongholds/analyses_misc/',folder,'/',folder,'_m90in_',season,'.img',sep=''), overwrite=TRUE)
# stop('cbw')
...
s90 <- reclassify(single.spp, rcl=reclass)
